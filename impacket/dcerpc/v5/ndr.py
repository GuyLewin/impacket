...
class NDR(object):
    ...
    def pack(self, fieldName, fieldTypeOrClass, soFar = 0):
        ...
        if len(fieldTypeOrClass) == 2 and fieldTypeOrClass[1] == 'L':
            data = int(data)

        return pack(fieldTypeOrClass, data).decode("latin1")
...
class NDRCONSTRUCTEDTYPE(NDR):
        ...
        for fieldName, fieldTypeOrClass in self.referent:
            try:
                if isinstance(self.fields[fieldName], NDRUniConformantArray) or isinstance(self.fields[fieldName], NDRUniConformantVaryingArray):
                    ...
                    data = arrayPadding + pack(arrayPackStr, self.getArrayMaximumSize(fieldName)).decode("latin1") + data
                else:
                    pad = self.calculatePad(fieldTypeOrClass, soFar)
                    if pad > 0:
                        soFar += pad
    ...
    def getArraySize(self,fieldName, data, soFar):
        ...
        if isinstance(self.fields[fieldName], NDRUniConformantArray):
            ...
            arraySize = unpack(arrayUnPackStr, data[:arrayItemSize].encode("latin1"))[0]
            ...
            maximumCount = unpack(arrayUnPackStr, data[:arrayItemSize].encode("latin1"))[0]
            # Let's store the Maximum Count for later use
            self.fields[fieldName].fields['MaximumCount'] = maximumCount
            # Unpack the Actual Count
            arraySize = unpack(arrayUnPackStr, data[arrayItemSize*2:][:arrayItemSize].encode("latin1"))[0]
        else:
            # NDRUniVaryingArray Array
            arraySize = unpack(arrayUnPackStr, data[arrayItemSize:][:arrayItemSize].encode("latin1"))[0]
        ...
        if len(two) == 2:
            ...
            for each in self.fields[fieldName]:
                ...
                if dataClass is None:
                    if type(each) is str:
                        each = each.encode("latin1")
                    answer += pack(item, each).decode("latin1")
                else:
        ...
        if len(two) == 2:
            ...
            while numItems and soFarItems < len(data):
                ...
                if dataClassOrCode is None:
                    ...
                    answer.append(unpack(item, data[soFarItems:nsofar].encode("latin1"))[0])
                else:
            ...
            try:
                if isinstance(self.fields[fieldName], NDRUniConformantArray) or isinstance(self.fields[fieldName], NDRUniConformantVaryingArray):
                    ...
                    if isinstance(self, NDRPOINTER):
                        ...
                        data = pointerData + arrayPadding + pack(arrayPackStr ,self.getArrayMaximumSize(fieldName)).decode("latin1") + data
                    else:
                        data = arrayPadding + pack(arrayPackStr, self.getArrayMaximumSize(fieldName)).decode("latin1") + data
                    arrayPadding = ''
                    arrayItemSize = 0
...
class NDRPOINTER(NDRSTRUCT):
    ...
    def fromString(self,data,soFar = 0):
        ...
        if unpack(unpackStr, data[:calcsize(unpackStr)].encode("latin1"))[0] == 0:
            ...
            try:
                ...
                if isinstance(self.fields[fieldName], NDRUniConformantArray) or isinstance(self.fields[fieldName],
                    ...
                    if self._isNDR64:
                        ...
                        pad = (4 - (soFar % 4)) % 4
                        data += '\xce'*pad + pack('<L', arraySize).decode("latin1") + res
                else:
                    data += self.pack(fieldName, fieldTypeOrClass, soFar)
